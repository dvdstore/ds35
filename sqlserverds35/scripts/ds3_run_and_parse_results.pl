...
foreach my $i (0 .. $#threads_list) {
  ...
  #`start cmd /c plink -pw ca\$hc0w root\@10.133.255.203 mpstat 10 36 ^>  ${threads_list[$i]}thread${title}mpstat.out\n`;
  system("start cmd /c plink -pw ca\$hc0w root\@10.133.255.203 mpstat 10 36 ^>  ${threads_list[$i]}thread${title}mpstat.out");
  system("start cmd /c plink -pw ca\$hc0w root\@10.133.255.203 iostat -d -x sda 10 36 ^> ${threads_list[$i]}thread${title}iostat.out");
  foreach my $j (1 .. ($num_drivers-1)){
    print  "start cmd /c ds3oracledriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv\n"; 
    system("start cmd /c ds3oracledriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv");
  }
  print "ds3oracledriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv\n";
  `ds3oracledriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv`;
...
foreach my $j (0 .. $#threads_list) {
  ...
  foreach my $i (4 .. $#iostatlines) {
    ...
    if (($#iostatvalues >= 0) && ($iostatvalues[0] eq 'sda')) {
