...
my $esxtopVMcmdsAvg;
my $esxtopVMreadsAvg;
my $esxtopVMwritesAvg;
my $esxtopVMMBreadAvg;
my $esxtopVMMBwriteAvg;
my $esxtopVMsecPerReadAvg;
my $esxtopVMsecPerWriteAvg;
...
$outputcsv = $title . 'with_disk_results.csv';

open (my $OUT, ">$outputcsv") || die("Can't open $outputcsv");

print $OUT "Threads,OPM,ART,CPU,ESXHOST1CPU,ESXHOST2CPU,ESXHOST3CPU,ESXHOST4CPU,Commands/sec,Reads/sec,Writes/sec,MBytes Read/sec,MBytes Written/sec,Average MilliSec/Read,Average MilliSec/Write\n";  #output file header
...
foreach my $j (0 .. $#threads_list) {
  ...
  my $ds3esxtopfilename_vmdisk0 = $threads_list[$j] . 'threads' . $title . 'esxtophost_VMdisk0.csv';
  
  print "$ds3esxtopfilename_vmdisk0\n";
  ...
  #Seperate out just the VM disk stats from the esxtop file 
  system("start cmd /c perl get_esxtop_vm_diskstats.pl ds3esxtopfilename1 $VMname ^> $ds3esxtopfilename_vmdisk0");
  
  #Get esxtop disk stats for vm - need to know which esxtop file to look in.
  $esxtopVMcmdsAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 0`;
  $esxtopVMreadsAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 1`;
  $esxtopVMwritesAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 2`;
  $esxtopVMMBreadAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 3`;
  $esxtopVMMBwriteAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 4`;
  $esxtopVMsecPerReadAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 5`;
  $esxtopVMsecPerWriteAvg = `perl get_esxtop_avg_column.pl $ds3esxtopfilename_vmdisk0 6`;
