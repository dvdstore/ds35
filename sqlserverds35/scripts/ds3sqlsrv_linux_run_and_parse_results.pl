...
my @threads_list = (5,6,7,8,9,10,11);
my $num_drivers = 10;
my $ds3_target = "10.133.255.188";
...
foreach my $i (0 .. $#threads_list) {
  ...
  system("start cmd /c plink -pw ca\$hc0w root\@${ds3_target} iostat -d -x sdb 10 36 ^> ${threads_list[$i]}thread${title}iostat.out");
 

  foreach my $j (1 .. ($num_drivers-1)){
    print  "start cmd /c ds3sqlserverdriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv\n"; 
    system("start cmd /c ds3sqlserverdriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv");
  }
  print "ds3sqlserverdriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv\n";
  `ds3sqlserverdriver.exe --config_file=param.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv`;
