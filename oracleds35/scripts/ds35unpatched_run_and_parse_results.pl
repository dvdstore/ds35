...
my @threads_list = (30);
my $num_drivers = 2;
my $ds3_target = "10.133.255.192";
my $esx_target = "w1-outperf27.eng.vmware.com";
my $intervals = 108;  # number of stats collection intervals 
...
foreach my $i (0 .. $#threads_list) {
  ...
  print "start cmd /c plink -pw ca\$hc0w root\@${ds3_target} mpstat ${delay} ${intervals} ^>  ${threads_list[$i]}thread${title}mpstat.out\n";   
  #`start cmd /c plink -pw ca\$hc0w root\@10.133.255.192 mpstat 10 36 ^>  ${threads_list[$i]}thread${title}mpstat.out\n`;
  system("start cmd /c plink -pw ca\$hc0w root\@${ds3_target} mpstat ${delay} ${intervals} ^>  ${threads_list[$i]}thread${title}mpstat.out");
  system("start cmd /c plink -pw ca\$hc0w root\@${ds3_target} iostat -d -x sdb sdc ${delay} ${intervals} ^> ${threads_list[$i]}thread${title}iostat.out");
  ...
  foreach my $j (1 .. ($num_drivers-1)){
    print  "start cmd /c ds35oracledriver.exe --config_file=param35_vm1.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv\n"; 
    system("start cmd /c ds35oracledriver.exe --config_file=param35_vm1.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv");
  }
  print "ds35oracledriver.exe --config_file=param35_vm1.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv\n";
  `ds35oracledriver.exe --config_file=param35_vm1.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv`;
