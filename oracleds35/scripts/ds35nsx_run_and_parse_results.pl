...
foreach my $i (0 .. $#threads_list) {
  ...
  foreach my $j (1 .. ($num_drivers-1)){
    print  "start cmd /c ds35oracledriver.exe --config_file=param35nsx.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv\n"; 
    system("start cmd /c ds35oracledriver.exe --config_file=param35nsx.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${j}.csv");
  }
  print "ds35oracledriver.exe --config_file=param35nsx.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv\n";
  `ds35oracledriver.exe --config_file=param35nsx.txt --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv`;
