...
my @paramfile_list = ("param1.txt","param2.txt","param3.txt","param4.txt","param1.txt","param1.txt","param1.txt","param1.txt");
my $num_ds3_targets = 4;
my @ds3_target_list = ("10.10.100.51","10.10.100.53","10.10.100.54","10.10.100.52","10.10.100.55","10.10.100.56","10.10.100.57","10.10.100.58");
...
foreach my $i (0 .. $#threads_list) {
  ...
  foreach my $j (0 .. $#esx_target_list){
    print  "start cmd /c plink -pw ca\$hc0w root\@${esx_target_list[${j}]} esxtop -b -d ${delay} -n ${intervals} ^> ${title}esxtop${j}.csv\n";
    system("start cmd /c plink -pw ca\$hc0w root\@${esx_target_list[${j}]} esxtop -b -d ${delay} -n ${intervals} ^> ${title}esxtop${j}.csv");
  ...
  `ds35sqlserverdriver.exe --config_file=${paramfile_list[$num_drivers-1]} --n_threads=${threads_list[$i]} --out_filename=${threads_list[$i]}threads${title}${num_drivers}.csv`;
