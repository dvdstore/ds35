 ...
 *
 * Support for PHP 7.4 and mysql1
 *
...
if (!(empty($productid) OR empty($review_stars) OR empty($review_summary) OR empty($review_text)))
  ...
  $new_review_proc_call = "call DS3.NEW_PROD_REVIEW$storenum(" .
  "'$productid','$review_stars','$customerid','$review_summary','$review_text', @reviewid_out);";
  mysqli_query($link_id,$new_review_proc_call);
  ...
  $get_title_result = mysqli_query($link_id,$get_product_title_query);
  $get_title_result_row = mysqli_fetch_row($get_title_result);
  $producttitle = $get_title_result_row[0];
  mysqli_free_result($get_title_result);
