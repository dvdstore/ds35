...
if (empty($confirmpurchase))
  ...
  for ($i=0; $i<count($item); $i++)
    ...
    if (empty($drop) || !in_array($i, $drop))
      {
      ++$j;
      $purchase_query = "select PROD_ID, TITLE, ACTOR, PRICE from DS3.PRODUCTS$storenum where PROD_ID=$item[$i]";
/*      mysqli_real_query($link_id, $purchase_query);*/
      $purchase_result = mysqli_query($link_id,$purchase_query);
  ...
  // loop through items to be purchased, check inventory,  and make inserts into ORDERLINES and CUST_HIST tables     
    ...
    if ($success)
      ...
      $cc_query = "select CREDITCARDTYPE, CREDITCARD, CREDITCARDEXPIRATION from DS3.CUSTOMERS$storenum where CUSTOMERID=$customerid";
      $cc_result = mysqli_query($link_id,$cc_query);
