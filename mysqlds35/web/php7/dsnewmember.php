...
if (!(empty($membershiplevel)))
  ...
  $query = "select COUNT(*) from DS3.MEMBERSHIP$storenum where CUSTOMERID='$customerid';";
...
/*  $result = mysqli_store_result($link_id);*/
  ...
  if ($row[0] != 0)
    ...
    if (($membershiplevel >= 1) AND ($membershiplevel <= 3))
      {
      $new_member_proc_call = "call DS3.NEW_MEMBER$storenum(" .
      "'$customerid','$membershiplevel', @customerid_out);";
      //echo $new_member_proc_call;
      mysqli_query($link_id,$new_member_proc_call);
