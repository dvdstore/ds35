...
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country) 
  ...
  $query = "select COUNT(*) from DS3.CUSTOMERS$storenum where USERNAME='$username';";
...
/*  $result = mysqli_store_result($link_id);*/
  ...
  if ($row[0] != 0)
    ...
    $new_customer_proc_call = "call DS3.NEW_CUSTOMER$storenum(" .
    ...
    mysqli_query($link_id,$new_customer_proc_call);
