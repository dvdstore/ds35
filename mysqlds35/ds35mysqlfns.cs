...
// 
    public bool ds2login(string username_in, string password_in, ref int customerid_out, ref int rows_returned, 
      ...
      db_query="select CUSTOMERID FROM DS3.CUSTOMERS"+ target_store_number + " where USERNAME='" + username_in + "' and PASSWORD='" + password_in
        + "';";
      ...
      try 
        ...
        Rdr.Close();
        db_query = "select PROD_ID from DS3.CUST_HIST" + target_store_number + " where CUSTOMERID =" + customerid_out + " ORDER BY ORDERID DESC LIMIT 10;"; 
        Login = new MySqlCommand(db_query, objConn);
        ...
        if (!Rdr.HasRows)  // No previous order
          ...
          while(Rdr.Read())
            ...
            string db_query2 = "select TITLE, ACTOR from DS3.PRODUCTS" + target_store_number + " where PROD_ID=" + prod_id + ";";
            ...
            Rdr2.Close();
            string db_query3 = "select TITLE from DS3.PRODUCTS" + target_store_number + " where PROD_ID = " +
              "(select COMMON_PROD_ID from DS3.PRODUCTS" + target_store_number + " where PROD_ID=" + prod_id + ");";
...
// 
    public bool ds2browse(string browse_type_in, string browse_category_in, string browse_actor_in,
      ...
      switch(browse_type_in)
        {
        case "title":
                db_query = "select * from PRODUCTS" + target_store_number + " where MATCH (TITLE) AGAINST ('" + browse_title_in + "') LIMIT " +
            batch_size_in + ";";
          break;
        case "actor":
          db_query = "select * from PRODUCTS" + target_store_number + " where MATCH (ACTOR) AGAINST ('" + browse_actor_in + "') LIMIT " +
            batch_size_in + ";";
          break;
        case "category":
          db_query = "select * from PRODUCTS" + target_store_number + " where CATEGORY = " + Convert.ToInt32(browse_category_in) + 
...
//      browse_title_in, browse_actor_in); 
      ...
      ref string[] review_summary_out, ref string[] review_text_out, ref int[] review_helpfulness_sum_out, ref double rt)
    {
      ...
      long ctr0 = 0, ctr = 0, freq = 0;
        ...
        {
            case "actor":
                db_query = "select T1.prod_id, T1.title, T1.actor, REVIEWS_HELPFULNESS" + target_store_number + ".REVIEW_ID, T1.review_date, T1.stars, " +
      ...
      ref string[] review_summary_out, ref string[] review_text_out, ref int[] review_helpfulness_sum_out, ref double rt)
    {
      ...
      long ctr0 = 0, ctr = 0, freq = 0;
        ...
        {
            case "noorder":
                db_query = "SELECT REVIEWS" + target_store_number + ".review_id, REVIEWS" + target_store_number + ".prod_id, REVIEWS" + target_store_number + ".review_date, REVIEWS" + target_store_number + ".stars, " +
                    ...
                    "WHERE PROD_ID = " + get_review_prod_in + " GROUP BY REVIEWS" + target_store_number + ".review_id ORDER BY total DESC;";
                break;
            case "star":
                db_query = "SELECT REVIEWS" + target_store_number + ".review_id, REVIEWS" + target_store_number + ".prod_id, REVIEWS" + target_store_number + ".review_date, REVIEWS" + target_store_number + ".stars, " +
                    "REVIEWS" + target_store_number + ".customerid,REVIEWS" + target_store_number + ".review_summary, REVIEWS" + target_store_number + ".review_text, SUM(REVIEWS_HELPFULNESS" + target_store_number + ".helpfulness) " +
                    ...
                    " GROUP BY REVIEWS" + target_store_number + ".review_id ORDER BY total DESC;";
                break;
            case "date":
                db_query = "SELECT REVIEWS" + target_store_number + ".review_id, REVIEWS" + target_store_number + ".prod_id, REVIEWS" + target_store_number + ".review_date, REVIEWS" + target_store_number + ".stars, " +
                    "REVIEWS" + target_store_number + ".customerid,REVIEWS" + target_store_number + ".review_summary, REVIEWS" + target_store_number + ".review_text, SUM(REVIEWS_HELPFULNESS" + target_store_number + ".helpfulness) " +
                    "as total FROM REVIEWS" + target_store_number + " INNER JOIN REVIEWS_HELPFULNESS" + target_store_number + " on REVIEWS" + target_store_number + ".review_id=REVIEWS_HELPFULNESS" + target_store_number + ".review_id " +
                    "WHERE PROD_ID = " + get_review_prod_in + " GROUP BY REVIEWS" + target_store_number + ".review_id ORDER BY REVIEW_DATE DESC;";
                break;
        }
...
// 
    public bool ds2purchase(int cart_items, int[] prod_id_in, int[] qty_in, int customerid_out,
      ...
      db_query = String.Format("INSERT into DS3.ORDERS" + target_store_number + " (ORDERDATE, CUSTOMERID, NETAMOUNT, TAX, TOTALAMOUNT) VALUES" + 
...
//        Console.WriteLine("Thread {0}: Purchase: neworderid_out= {1}", Thread.CurrentThread.Name, neworderid_out);
          ...
          string o_insert_query = "INSERT into DS3.ORDERLINES" + target_store_number + " (ORDERLINEID, ORDERID, PROD_ID, QUANTITY, ORDERDATE)" +
            " VALUES ";
          string c_insert_query = "INSERT into DS3.CUST_HIST" + target_store_number + " (CUSTOMERID, ORDERID, PROD_ID) VALUES "; 
        
          for (i=0; i<cart_items; i++)
            {
            j = i+1;
            db_query = "SELECT QUAN_IN_STOCK, SALES FROM DS3.INVENTORY" + target_store_number + " WHERE PROD_ID=" + prod_id_in[i] + ";";
            Purchase = new MySqlCommand(db_query, objConn);
            Rdr = Purchase.ExecuteReader();
            Rdr.Read();
            ...
            if (new_quan < 0)
              ...
              {
                  db_query = "UPDATE DS3.INVENTORY" + target_store_number + " SET QUAN_IN_STOCK=" + new_quan + ", SALES=" + 
