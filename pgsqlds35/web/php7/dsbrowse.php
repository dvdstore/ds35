...
<?php
/*  
 * DVD Store Browse PHP Postgresql Page - dsbrowse.php
 *
...
if (!empty($browsetype))
  ...
  $browse_result = pg_query($link_id,$browse_query);
  ...
  if (pg_num_rows($browse_result) == 0)
    ...
    while ($browse_result_row = pg_fetch_row($browse_result))
      ...
      echo "<TD><a href='dsgetreviews.php?customerid=$customerid&storenum=$storenum&review_title=$browse_result_row[2]&productid=$browse_result_row[0]' target='_blank'>$browse_result_row[2]</a></TD>\n";
...
if (isset($item))  // Show shopping cart
  ...
  for ($i=0; $i<count($item); $i++) 
    {
    $j=$i+1;
    $query = "select TITLE from PRODUCTS$storenum where PROD_ID=$item[$i];";
    $result = pg_query($link_id,$query);
    $result_row = pg_fetch_row($result);
    $title = $result_row[0];
    echo "<TD>$j</TD><TD><a href='dsgetreviews.php?customerid=$customerid&storenum=$storenum&productid=$item[$i]&review_title=$title' target='_blank'>$title</a></TD></TR>\n";
    }
