...
<?php
/*  
 * DVD Store Get Reviews PHP Postgresql Page - dsgetreviews.php
 *
 * Copyright (C) 2005 Dell, Inc. <davejaffe7@gmail.com> and <tmuirhead@vmware.com>
 *
 * Gets Reviews of products in Postgresql DVD store by author, title, product id, date, and star ranking
 *
...
if (!empty($getreviewtype))
  ...
  switch ($getreviewtype)
    {
    case "noorder":
      $getreview_query = "SELECT * from get_prod_reviews$storenum ($limit_num, $productid);";
      break;
    case "date":
      $getreview_query ="SELECT * from get_prod_reviews_by_date$storenum ($limit_num, $productid);";
      break;
    case "star":
      $getreview_query = "SELECT * from get_prod_reviews_by_stars$storenum ($limit_num, $productid, $review_stars);";
      break;
    }

  $getreviews_result = pg_query($link_id,$getreview_query);

  if (pg_num_rows($getreviews_result) == 0)
    {
    ...
    while ($getreviews_result_row = pg_fetch_row($getreviews_result))
      {
      echo "----------------------------------------------------------------------------------------------<BR>";
    ...
    pg_free_result($getreviews_result);
