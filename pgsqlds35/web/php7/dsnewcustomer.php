...
<?php
/*  
 * DVD Store New Customer PHP Posgtresql Page - dsnewcustomer.php
 *
 * Copyright (C) 2005 Dell, Inc. <davejaffe7@gmail.com> and <tmuirhead@vmware.com>
 *
 * Prompts for new customer data; creates new entry in Postgresql DVD Store CUSTOMERS table
 *
...
$storenum = isset($_REQUEST["storenum"]) ? $_REQUEST["storenum"] : NULL;
...
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country) 
  ...
  $query = "select COUNT(*) from CUSTOMERS$storenum where USERNAME='$username';";
  ...
  if ($row[0] != 0)
    ...
    $new_customer_proc_call = "select new_customer$storenum(" .
    ...
    $result = pg_query($link_id,$new_customer_proc_call);
    $row = pg_fetch_row($result);
    $customerid = $row[0];
    pg_free_result($result);
  ...
  dsnewcustomer_form($firstname,$lastname,$address1,$address2,$city,$state,$zip,$country,
    $email,$phone,$creditcardtype,$creditcard,$ccexpmon,$ccexpyr,$username,$password,$age,$income,$gender,$storenum);
  }
...
function dsnewcustomer_form($firstname,$lastname,$address1,$address2,$city,$state,$zip,$country,
  ...
  echo "Store Number <INPUT TYPE=TEXT NAME='storenum' VALUE='$storenum' SIZE=3 MAXLENGTH=3> <BR>\n";
