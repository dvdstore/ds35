...
<?php
/*  
 * DVD Store New Member PHP Postgresql Page - dsnewmember.php
 *
 * Copyright (C) 2005 Dell, Inc. <davejaffe7@gmail.com> and <tmuirhead@vmware.com>
 *
 * Prompts for new member data; creates new entry in Postgresql DVD Store MEMBERSHIP table
 *
...
if (!(empty($membershiplevel)))
  ...
  if ($row[0] != 0)
    ...
    if (($membershiplevel >= 1) AND ($membershiplevel <= 3))
      {
      $new_member_proc_call = "select new_member$storenum( $customerid, $membershiplevel );";
      $result = pg_query($link_id,$new_member_proc_call);
      $row = pg_fetch_row($result);
      $customerid = $row[0];
      pg_free_result($result);
